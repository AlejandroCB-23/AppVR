...
public class EyeDataCollector : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (EyeManager.Instance != null) // Verificar si tenemos en la escena un objeto de seguimiento ocular
        {
            ...
            CheckEyeTrackingAvailability(); // Verificar si el seguimiento ocular est� disponible
        }

        InitializeLineRenderer(); // Inicializar el renderizador de l�neas
    }

    void Update()
    {
        // Verificar si el seguimiento ocular est� disponible
    ...
    void CaptureEyeTrackingData()
    {
        ...
        if (leftEyeDataAvailable || rightEyeDataAvailable || combinedEyeDataAvailable)
        {
            ...
            // Transformaci�n de las coordenadas de los ojos
            ...
            // Direcci�n de los ojos en el mundo
    ...
    void PerformRaycastWithEyeData(Vector3 eyeTransformed, Vector3 eyeDirection)
    {
        ...
        {
            DrawEyeRay(eyeTransformed, eyeTransformed + eyeDirection * maxDistance);  // Dibujar rayo sin colisi�n
        }
    }
    ...
    void UpdateObjectColor(GameObject obj)
    {
        ...
        {
            // Obt�n el n�mero de impactos acumulados con el objeto
            int hitCount = heatMap.ContainsKey(obj) ? heatMap[obj] : 0;

            // Escala de impacto que define cu�ntos impactos se necesitan para que el objeto cambie de color
            float impactScale = 125f; // Aumenta este valor para que se necesiten m�s impactos

            // Calcula la intensidad con un cambio gradual (ajustado por la escala de impactos)
            float intensity = Mathf.Clamp01(Mathf.Sqrt(hitCount) / impactScale);  // Se necesita m�s hits para llegar a 1
    ...
    // Funci�n para obtener el color correspondiente a la intensidad
    ...
    {
        // Usamos una lista local para evitar acceder a la cola en cada iteraci�n.
        ...
        await File.AppendAllTextAsync(normalDataFilePath, normalJsonData);  // Asincr�nico, no bloquea el hilo principal
        ...
        await File.AppendAllTextAsync(transformedDataFilePath, transformedJsonData);  // Asincr�nico
    }
    ...
    void SaveHeatMapData()
    {
        ...
        File.WriteAllText(heatMapFilePath, heatMapJsonData);  // Escribir de manera s�ncrona
    }
